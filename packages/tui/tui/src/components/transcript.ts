...
import { DEEPSEEK_LOGO, needsAsciiLogo, selectLogoTier } from './logo.ts'
...
/**
 * Borderless startup banner: the official DeepSeek mark, the product title, an
 * optional configured subtitle, and the session id. No box frame — each line
 * renders as plain left-padded text (matching transcript notices) so it reads
 * on any theme. The mark drops to a smaller raster, then out entirely, as the
 * terminal shrinks, so the banner never wraps or crowds the transcript.
 */
export class HeaderComponent implements Component {
  ...
  /** Terminal rows of the official mark, already inked, or none at this size. */
  private markLines(usable: number): string[] {
    const tier = selectLogoTier(usable, this.viewportRows())
    if (tier === undefined) return []
    const logo = DEEPSEEK_LOGO[tier]
    const rows = needsAsciiLogo() ? logo.ascii : logo.unicode
    // The mark is trusted static art with no controls of its own, so it takes
    // the brand ink directly on truecolor terminals and the accent role
    // otherwise, where the host theme owns the exact hue.
    const ink = this.gradient ? brandText : this.palette.accent
    return [...rows.map(row => ink(row)), '']
  }
  ...
  render(width: number): string[] {
    ...
    const lines = [
      // Raster rows are fixed-width art: wrapping or re-inking them would break
      // the mark, so they bypass the text pipeline and only take the gutter.
      ...this.markLines(usable),
      ...[
        title,
