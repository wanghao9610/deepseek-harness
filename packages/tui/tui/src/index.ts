...
/**
 * Timestamp of the newest event that records work, for the `/status` snapshot.
 * `session/end-seed` is the boundary a seeded load appends, not activity.
 * @param events - the session log to scan, in seq order.
 * @returns the latest non-boundary event's `time`, or `undefined` when there is none.
 */
function lastActivityTime(events: readonly SessionEvent[]): number | undefined {
  return events.findLast(event => event.type !== 'session/end-seed')?.time
}

/**
 * Terminal notice for one durable cancellation cause.
 * @param cause - the cause recorded on an `aborted` turn end.
 * @returns the transcript notice text.
 */
function cancelCauseNotice(cause: TurnEndCancelCause): string {
  switch (cause.kind) {
    case 'disposed':
      return 'Turn stopped: the agent was disposed.'
    case 'hook':
      return `Turn stopped by a hook: ${cause.reason}`
    case 'parent':
      return 'Turn cancelled by the parent agent.'
    default:
...
export function createTuiChat(
  ...
  // Turns whose failure a live `agent/error` notice already presented, so the
  // durable `turn/end` reason does not repeat it.
      ...
      case 'turn/end': {
        ...
        switch (reason.kind) {
          case 'completed':
            break
          case 'error':
            // A live `agent/error` for this turn already named the failure;
            // the durable reason repeats it, so present it only once.
            if (!liveErrors.delete(event.data.turn)) appendNotice(reason.error.message, 'error')
            break
          case 'aborted':
            appendNotice(cancelCauseNotice(reason.reason), 'warning')
            break
          case 'blocked':
            appendNotice('Turn blocked before any model request.', 'warning')
            break
          case 'max-tokens':
  ...
  const dispatchMessage = (content: ContentBlock[], attachedContext?: UserMessage): void => {
    ...
    if (agent.status === 'running') {
      // Steering is never subject to step admission; an attached snapshot
      // drains beside it at the same step boundary through the inbox.
    ...
    // Idle: the snapshot rides the prompt's own step admission so a rejecting
    // listener drops both together.
    let cleanedUp = false
    ...
    // Prepended so this wrapper is outermost: it observes the step that claimed
    // the exact accepted message whether a downstream listener enters or
    // rejects it, then detaches.
    const detachPreStep = ctx.on('agent/pre-step', async (payload, next) => {
      if (payload.agent !== agent || !payload.messages.some(claimed => claimed.id === acceptedId)) return next()
      cleanup()
      const decision = await next()
      if (decision.kind !== 'enter') return decision
      return { ...decision, messages: [...decision.messages, attachedContext] }
    }, { prepend: true })
    // Installed before followup(): a delivery listener can synchronously
    // cancel and discard before followup() returns.
    const detachDiscard = ctx.on('agent/inbox/discarded', (payload) => {
      if (payload.agent === agent && payload.message.id === acceptedId) cleanup()
    })
  ...
  return {
    async dispose(): Promise<void> {
      // Retire the channel BEFORE detaching, matching the startup-failure path
      // above. `detachListeners` disposes the prompt-value handles, so an
      // in-flight async callback landing between the two would pass
      // `requestRender`'s guard and then write to a disposed handle.
      disposed = true
      detachListeners()
