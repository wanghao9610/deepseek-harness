...
import { CombinedAutocompleteProvider, visibleWidth, type Terminal } from '@deepseek-ai/tui'
...
/**
 * Mount one TUI over a fake terminal.
 * @param options - harness options.
 * @param rows - terminal height; scenarios asserting on a long card that the
 * startup banner would push above the fold pass a taller terminal, since
 * pi-tui only writes the visible viewport.
 * @returns the mounted harness.
 */
async function setup(options: TuiHarnessOptions = {}, rows?: number) {
...
describe('pi-tui chat lifecycle and transcript', () => {
  ...
  it('renders its header, footer, replay, streaming answer, todos, and status', async () => {
    ...
    // Steering enters the durable log as an ordinary user message, so the
    // transcript gives it the same `You` header as any other prompt.
  ...
  it('badges queued steering on the prompt context and clears it as each drains', async () => {
    ...
    for (const message of discarded) agentEvents(result.ctx, other).emit('agent/inbox/discarded', { message })
    ...
    expect(result.terminal.output).toContain('2 queued')
    result.terminal.output = ''
    for (const message of discarded) agentEvents(result.ctx, result.agent).emit('agent/inbox/discarded', { message })
    await tick()
    expect(result.terminal.output).not.toContain('queued')
  ...
  it('shows a live standalone compaction in the fixed status area', async () => {
    ...
    result.session.append('compaction/start', { compactionId: CompactionId('test-compaction'), turn: null })
    clock = 1_000
    result.terminal.output = ''
  ...
  it('ignores a numbered compaction bracket while the status line is idle', async () => {
    ...
    result.session.append('compaction/start', { compactionId: CompactionId('test-compaction'), turn: 1 })
    await tick()
  ...
  it('fades a closed standalone compaction back to the plain caret', async () => {
    ...
    result.session.append('compaction/start', { compactionId: CompactionId('test-compaction'), turn: null })
    await tick()
    result.session.append('compaction/end', { compactionId: CompactionId('test-compaction'), turn: null })
    await tick()
  ...
  it('reports a failed standalone compaction when its live bracket closes', async () => {
    ...
    result.session.append('compaction/start', { compactionId: CompactionId('test-compaction'), turn: null })
    result.terminal.output = ''
    result.session.append('compaction/end', { compactionId: CompactionId('test-compaction'), turn: null, error: 'summary failed' })
    await tick()
  ...
  it('does not show compaction progress for a resumed orphaned start', async () => {
    const result = await setup({
      ...
      now: () => 1_000,
      beforeMount(session) {
        session.append('compaction/start', { compactionId: CompactionId('test-compaction'), turn: null })
      },
    })
  ...
  it('combines session autocomplete with files and prepares send/steer references asynchronously', async () => {
    ...
    const decision = await agentEvents(result.ctx, result.agent).waterfall(
      'agent/pre-step',
      { messages: [result.agent.sentMessages[0]!], turn: 1, step: 1, signal: new AbortController().signal },
    ...
    expect(replay.kind === 'enter' && replay.messages).toHaveLength(1)
  ...
  it('releases the reference-admission wrapper on the ordinary allowed path', async () => {
    ...
    const unrelated = await agentEvents(result.ctx, result.agent).waterfall(
      ...
      () => Promise.resolve({ kind: 'enter' as const, messages: [createUserMessage({
        content: [{ type: 'text', text: 'unrelated' }],
        source: { kind: 'user' },
      })] }),
    )
    expect(unrelated.kind === 'enter' && unrelated.messages).toHaveLength(1)
  ...
  it('releases the reference wrapper when enqueue synchronously discards before followup returns', async () => {
    ...
    result.agent.followup = (input) => {
      ...
      agentEvents(result.ctx, foreign).emit('agent/inbox/inserted', { message: message })
      agentEvents(result.ctx, result.agent).emit('agent/inbox/inserted', { message: message })
      agentEvents(result.ctx, result.agent).emit('agent/inbox/discarded', { message: message })
      return message.id
    }
  ...
  it('discards the reference snapshot with its blocked or cancelled prompt', async () => {
    ...
    expect(passthrough.kind === 'enter' && passthrough.messages).toHaveLength(1)
    ...
    agentEvents(result.ctx, result.agent).emit('agent/inbox/discarded', { message: createUserMessage({
    ...
    expect(afterDiscard.kind === 'enter' && afterDiscard.messages).toHaveLength(1)
    await dispose(result)
  })
