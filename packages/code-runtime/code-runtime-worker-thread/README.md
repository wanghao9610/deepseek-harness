...
- **A failed program is located in its own coordinates** — an exception diagnostic keeps the thrown message and restates every frame V8 attributed to the program as `program:<line>:<column>`, counted from the model's first line by measuring the synthesized function header instead of assuming its size; frames belonging to this file, to Node internals, and to the absolute host paths they name are dropped. A rejected parse is located the same way from the stripper's own line marker and caret column, minus the strip wrapper's line, and reports no location at all when the parser stopped outside what the model wrote — at the brace the wrapper appended, where an unterminated program fails — rather than naming a line that does not exist.
...
Indirectly, through Code Mode in [`dsh-tools`](../../core/tools/README.md), which renders the exact outer value when it fits or an explicit `invalid-output` / `output-limit` failure. A thrown program error or a rejected parse reaches the model as its own message plus a `program:<line>:<column>` frame, so a retry targets the line the model wrote. Only the outer `run_code` result enters model context and its ordinary spill policy; binding traffic and intermediate values remain execution-local.
