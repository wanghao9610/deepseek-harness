...
| `resolveHarnessCheckout(moduleUrl)` | 包含调用方模块的 Harness 实现代码 checkout：沿其目录链向上查找 workspace 根 manifest（`@deepseek-ai/dsh-root`）得到；当该模块从已安装的包树运行时返回 `undefined`，因为那里的根 manifest 属于安装方应用。仅凭目录层数无法区分这两种布局：同一个包在 checkout 中位于 `packages/<group>/<pkg>/`，安装后则位于 `node_modules/@deepseek-ai/<pkg>/` |
| `addHarnessSourceSection(ctx, sourceRoot)` | 添加全局 `harness:source` 提示词段落（顺序紧随 harness 身份、位于 persona 之前），告知 agent（智能体）DSH 实现代码 checkout 的磁盘路径，同时提醒它不得据此推断当前工作目录，而应使用 `pwd`；如果已启动树没有此项服务，或者 `sourceRoot` 为 `undefined`（运行中的代码不在任何 checkout 内），则不执行操作并返回 `undefined`。这里的服务是 `systemPrompt`；该段落注册到它的 fiber，因此开发环境 HMR 重新加载系统提示词后，它会消失直至下次启动 |
