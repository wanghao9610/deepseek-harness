...
由 `apps/cli`、桌面外壳和 [`examples/`](../examples/README.md) demo bin 共享、与渠道无关的启动库。
...
| [`ssh-launch/`](ssh-launch/README.md) | 为经 SSH 由另一台主机提供服务的 runtime 做启动规划：连接校验、`ssh` 命令行及其远端脚本、失败诊断 | （供各外壳使用的库） |
