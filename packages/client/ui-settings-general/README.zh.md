...
面板可从触发控件打开，也可从平台自身的设置组合键打开——macOS 上是 `⌘,`，其他平台是 `Ctrl+,`——它经 [ui-primitives](../ui-primitives/README.md) 的 `useShortcut` 绑定到同一份打开状态上，因此面板已打开时再按一次仍保持打开，并停留在当前分区。关闭路径是标题栏按钮、遮罩点击与 Escape。触发控件的 tooltip 只标示该组合键，不含其他内容：在那里写出操作名意味着外壳持有文案，而这一行上的每个词都属于注册方。
