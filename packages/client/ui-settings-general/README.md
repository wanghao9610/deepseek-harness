...
The panel opens from the trigger row and from the platform's settings chord — `⌘,` on macOS, `Ctrl+,` elsewhere — bound through [ui-primitives](../ui-primitives/README.md)'s `useShortcut` on the same open state, so pressing it while the panel is up leaves it up on whichever section is showing. It closes through the header button, a mask click, or Escape. The trigger's tooltip announces that chord and nothing else: naming the action there would be the shell holding copy, and every word on this row is a registrant's.
