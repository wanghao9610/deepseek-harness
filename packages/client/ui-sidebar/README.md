...
The column also carries the New Session chord — `⌘K` on macOS, `Ctrl+K` elsewhere — and the collapse chord `⌘B` / `Ctrl+B`, both through [ui-primitives](../ui-primitives/README.md)'s `useShortcut`. They belong to the column rather than to either button, so the collapsed rail answers them exactly as the expanded column does; `⌘B` toggles rather than collapses, because the rail has no second key to come back with and the button under it is the same one in both states. The capsule's tooltip names it (`New session ⌘K`) in both column states rather than on the rail alone, because the chord is something the expanded button's own label does not carry.
