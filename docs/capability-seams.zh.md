...
| `ctx.tui` | `core` | [`tui`](../packages/tui/tui) | - | - | - | 已挂载的终端前端让其他插件把键盘 overlay 排进它的模态队列，而无需够到 pi-tui、终端或焦点。 |
| `ctx.tuiPrompt` | `core` | [`tui`](../packages/tui/tui) | - | - | - | 插件注册具名片段供终端状态行插值，并随自身状态变化设置它们。 |
| `ctx.tuiResumeHost` | `core` | [`tui`](../packages/tui/tui) | - | [`tui-app`](../packages/bundle/tui-app) | - | 替换自身以载入所选会话的是持有进程而非插件，因此恢复的会话运行在它自己的工作区目录中。 |
