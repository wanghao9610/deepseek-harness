...
- The packaged-binary arm — the packaged executable spawning itself as the dialog entry — is not exercised by any automated test, because no lane packages the desktop application. What is covered is the source plane, the built `lib/worker.cjs` under plain node, and that bundle's whole COM conversation on win32 ([the worker-death note](../bug-fix/2026-08-14-win32-dialog-worker-death-reports-itself.md), which also records what a worker that dies before reporting now says).
