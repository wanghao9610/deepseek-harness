...
/**
 * Members that sit in a release directory but ship an installer artifact
 * instead of an npm package. Publication rules — public access, a repository
 * directory, a publication files policy — describe a distribution they do not
 * have, so they take the ordinary private-member rule instead.
 */
const artifactMemberDirectories = new Set(['apps/desktop'])
...
function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  } else if (releaseMemberDirectory.test(dir) && !artifactMemberDirectories.has(dir)) {
  ...
  if (dir.startsWith('apps/') && manifest.name?.startsWith('@deepseek-ai/') && !artifactMemberDirectories.has(dir)) {
