 ...
 * @param continuationWidth - Maximum visible width for every chunk after the first.
 *                            Defaults to `maxWidth`; a wider value lets wrapped rows
 *                            reclaim the columns a first-row prompt prefix occupies.
	...
	for (let i = 0; i < segments.length; i++) {
		...
		if (gWidth > currentMaxWidth) {
			if (segments.length === 1) {
				// The whole line is one atomic segment wider than the row: emit it
				// verbatim. Re-wrapping would recurse on the identical input.
				chunks.push({ text: grapheme, startIndex: charIndex, endIndex: charIndex + grapheme.length });
				return chunks;
			}

			// Single atomic segment wider than the current line width (e.g. paste
...
interface LayoutLine {
	...
	hasCursor: boolean;
	cursorPos?: number;
	/** True for a row produced by word-wrapping, false for the first row of a logical line. */
...
export class Editor implements Component, Focusable {
	...
	private frame: "horizontal" | "none" = "horizontal";
	private prompt?: { first: string; continuation: string };
	private promptWidth: number = 0;

	// Store last render width for cursor navigation
	private lastWidth: number = 80;
	private lastContinuationWidth: number = 80;
	 ...
	 *
	 * @param prompt - Prefixes whose visible widths must be equal, so every input
	 *                 row starts at the same column.
	 */
	...
	render(width: number): string[] {
		...
		for (let visibleIndex = 0; visibleIndex < visibleLines.length; visibleIndex++) {
			...
			// The first input row carries `prompt.first`, every later logical line
			// carries `prompt.continuation`, and a wrapped row carries neither so
			// it can reclaim the prefix columns.
	...
	private layoutText(contentWidth: number, continuationWidth: number): LayoutLine[] {
	 ...
	 *
	 * `continuationWidth` must match the width {@link render} last wrapped with,
	 * or cursor navigation lands on a different row than the one drawn.
	 */
